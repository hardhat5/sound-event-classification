...
import argparse
from utils import AudioDataset, Task5Model, Cnn10
...
def run(feature_type, num_frames, perm):
    ...
    folds = []
    for i in range(5):
        folds.append(pd.read_csv('./metadata/split_8k/fold_{}.txt'.format(i), delimiter=" ", header=None))
...
if __name__=="__main__":
    ...
    parser.add_argument('-f', '--feature_type', type=str, default='logmelspec_8k')
    parser.add_argument('-n', '--num_frames', type=int, default=1003)
    parser.add_argument('-p', '--permutation', type=int, nargs='+', default = [4,0,1,2,3])
